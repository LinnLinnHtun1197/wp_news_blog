...
function first_news_scripts() {
	...
	//google Fonts
	$first_news_google_fonts = array('Montserrat','Raleway','Lato');
	...
	wp_enqueue_script( 'moment-js', get_template_directory_uri() . '/assets/js/moment.js', array(), esc_attr( $theme_version ), true );
 ...
 * @since 1.1.4
 */
function first_news_default_category(){
	//get the category
	$categories = get_categories();

	foreach( $categories as $categorie ){
		$count = $categorie->category_count;
		if( $count > 5 ) {
			return $default_category = $categorie->term_id;
		} 
